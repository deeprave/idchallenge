...
def subst(environ, lines, quote=False) -> list:
    ...
    with open(template, 'r') as f:
        for line in f:
            ...
            if not line or line.startswith('#'):
                ...
                if len(parts) == 1:
                    var, val = parts[0], env.get(parts[0], '')
                else:
                    var, val = parts[0], env.get(parts[0], parts[1])
                yield var, val
...
def output_result(lines, outputfile):
    ...
    def writelines(fp, _lines):
        ...
        for linecount, line in enumerate(_lines):
            print(line, file=fp)
        print(f"{linecount+1} line(s) written", file=sys.stderr)
...
def prune_env(env, prefix):
    m = re.match(r'^_([A-Za-z0-9]+)_', prefix)
    if m:
        prefix = f'{m[1]}'
    prefix = f'_{prefix}_'
    l = len(prefix)
...
def build_command(args):
    env = read_env(args.dotenv, search=args.search, parents=args.parents, useenv=args.environ)
    # handle special _<target>_<variable>=<value>
    # rules:
    #   - by default ignore all vars that start with an underscore
    #   - include vars that start with our prefix, remove prefix overwrite
    env = prune_env(env, args.prefix)
    data = parse_template(env, args.template)
    rendered = subst(env, data, args.quote)
...
if __name__ == '__main__':
    ...
    build_parser.add_argument('-P', '--prefix', action='store', default='_PROD_',
                              help=f'override _VARS as env specific values')
    build_parser.add_argument('-q', '--quote', action='store_true', default=False,
                              help=f'quote values')
